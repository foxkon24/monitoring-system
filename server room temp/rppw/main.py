...
TEMP_ABNORMAL = float( 28.0 )  #float( 25.0 )  #異常温度(℃)
CNT_OVERFLOW = 10
...
#メール通知
def mail_notify( multiple_sent_enable = False ):
    global temp
    ...
    while True:
        ...
        if cnt >= CNT_OVERFLOW:
            ...
            if temp >= TEMP_ABNORMAL:  #異常温度以上
                await mail_notify( multiple_sent_enable = True )
...
if __name__ == "__main__":
    #初期化
    try:
        lcl_led = Pin( "LED", machine.Pin.OUT )
